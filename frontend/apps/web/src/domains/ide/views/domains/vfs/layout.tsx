...
import {Ide, LockedSpec, ProgressiveWorkspace} from "playground-core";
...
export const VfsLayout = (props: VfsLayoutProps): React.ReactElement => {
    ...
    const folder =
        props.phase == "locked" 
        && props.locked.workspace.kind == 'unstale' ?
    ...
    const file =
        props.phase == "locked"
        && props.locked.workspace.kind == 'unstale' && props.locked.workspace.current.kind == 'file'
            ? <MonacoEditor
    ...
    const drawer =
        ...
        ? <Drawer 
            ...
            onSelectedFolder={(folder) =>
                props.setState(LockedSpec.Updaters.Core.workspace(ProgressiveWorkspace.Updater.selectFolder(folder)))
            }
            onSelectedFile={(file) => 
                props.setState(LockedSpec.Updaters.Core.workspace(ProgressiveWorkspace.Updater.selectFile(file)))
            }
            drawerId="my-drawer" /> : <></>
