...
> = {
  ...
  table: {
    ...
    streamingTable2: () => (props) => {
      console.debug("streamingTable2", console.debug(props.TableData, null, 2));
      const [colFilterDisplays, setColFilterDisplays] = useState<
        Map<string, boolean>
      >(Map(props.AllowedFilters.map((_) => false)));
                    ...
                    .map(([id, row], idx) => {
                      ...
                      return (
                        <tr style={{ border: "1px solid black" }}>
                          ...
                          {props.context.tableHeaders.map((header: string) => (
                            <td style={{ border: "1px solid black" }}>
                              {(console.debug('xyz', header, row.get(header)), row.get(header)?.(undefined)({
                                ...props,
                                view: unit,
                              }))} 
                            </td>
