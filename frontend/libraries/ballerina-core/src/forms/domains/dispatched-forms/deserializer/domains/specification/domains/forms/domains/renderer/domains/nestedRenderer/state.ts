...
export const NestedRenderer = {
  Operations: {
    ...
    DeserializeAsInlineRenderer: <T>(
      ...
      !NestedRenderer.Operations.hasType(serialized)
        ...
        : MapRepo.Operations.tryFindWithError(
            ...
            .Then((type) =>
              NestedRenderer.Operations.IsRecordForm(serialized)
                ? type.kind == "record"
    ...
    DeserializeAs: <T>(
      ...
      NestedRenderer<T> | TableFormRenderer<T> | RecordFormRenderer<T>,
      string
    > => {
    ...
    > => {
      ...
      > = (() => {
        if (
          NestedRenderer.Operations.IsTableForm(serialized) ||
          NestedRenderer.Operations.IsRecordForm(serialized)
        ) {
          return NestedRenderer.Operations.DeserializeAsInlineRenderer(
            serialized,
            fieldViews,
            types,
          );
        ...
        if (
          NestedRenderer.Operations.IsSumUnitDate(serialized, fieldViews) &&
          type.kind == "sum"
        ) {
