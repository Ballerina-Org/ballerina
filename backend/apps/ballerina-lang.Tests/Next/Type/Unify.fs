...
let private valueOperations =
  { tryCompare =
      fun (v1: PrimitiveType, v2: PrimitiveType) ->
...
let ``LangNext-Unify binding trivial equivalence classes over primitives succeeds``
  ...
  let program
    : State<unit, _, EquivalenceClasses<string, PrimitiveType>, Errors<Location>> =
    state {
      do!
        EquivalenceClasses.Bind(
          "v1",
          PrimitiveType.Int32 |> Right,
          Location.Unknown
        )
        ...
        Map.ofList
          [ "v1",
            EquivalenceClass.Create(
              "v1" |> Set.singleton,
              PrimitiveType.Int32 |> Some
            )
            "v2",
            EquivalenceClass.Create(
              "v2" |> Set.singleton,
              PrimitiveType.String |> Some
            )
            "v3",
            EquivalenceClass.Create(
              "v3" |> Set.singleton,
              PrimitiveType.Decimal |> Some
            )
            "v4",
            EquivalenceClass.Create(
              "v4" |> Set.singleton,
  ...
  | _ ->
    Assert.Fail
      $"Expected a new state but:with equivalence classes but got none"
...
let ``LangNext-Unify binding equivalence classes over variables and primitives or variables fails``
  ...
  | Sum.Left((), result) ->
    Assert.Fail $"Expected unification to fail but it succeeded with {result}"
...
let ``LangNext-Unify unifies types without variables`` () =
  let inputs =
    [ TypeValue.CreateInt32()
      ...
      TypeValue.CreateTuple(
        [ TypeValue.CreateInt32(); TypeValue.CreateString() ]
      )
      TypeValue.CreateSum([ TypeValue.CreateInt32(); TypeValue.CreateString() ])
      TypeValue.CreateRecord(
        [ "a" |> Identifier.LocalScope |> TypeSymbol.Create,
          (TypeValue.CreateInt32(), Kind.Star)
          "b" |> Identifier.LocalScope |> TypeSymbol.Create,
  ...
  let actual =
    ...
    |> List.map (fun input ->
      TypeValue
        .Unify(Location.Unknown, input, input)
        .run (UnificationContext.Empty, UnificationState.Empty))
...
let ``LangNext-Unify unifies arrows`` () =
  ...
  match actual with
  | Sum.Left((), Some(actual)) ->
    Assert.That(actual.Classes, Is.EqualTo expected)
  | Sum.Right err -> Assert.Fail $"Expected success but got error: {err}"
  | _ -> ()
...
let ``LangNext-Unify unifies lists of tuples`` () =
  ...
  let inputs =
    Location.Unknown,
    TypeValue.CreateSet(
      TypeValue.CreateTuple([ TypeValue.Var(a); TypeValue.CreateString() ])
    ),
    TypeValue.CreateSet(
      TypeValue.CreateTuple([ TypeValue.Var(b); TypeValue.CreateString() ])
    )
  ...
  let expected: EquivalenceClasses<TypeVar, TypeValue<IComparable>> =
    { Classes =
        [ "a", EquivalenceClass.Create([ a; b ] |> Set.ofList, None) ]
        |> Map.ofList
...
let ``LangNext-Unify does not unify structurally identical but symbolically different records and unions``
  ...
  let inputs1 =
    Location.Unknown,
    TypeValue.CreateRecord(
      [ "s1" |> Identifier.LocalScope |> TypeSymbol.Create,
        (TypeValue.CreateString(), Kind.Star)
        "s2" |> Identifier.LocalScope |> TypeSymbol.Create,
        (TypeValue.CreateInt32(), Kind.Star) ]
      |> OrderedMap.ofList
    ),
    TypeValue.CreateRecord(
...
let ``LangNext-Unify does not unify structurally different tuples and sums``
  ...
  let inputs1 =
    Location.Unknown,
    TypeValue.CreateTuple(
      [ TypeValue.CreateString(); TypeValue.CreateDecimal() ]
    ),
    TypeValue.CreateTuple([ TypeValue.CreateString(); TypeValue.CreateInt32() ])
    ...
    (TypeValue
      ...
      .run (
        UnificationContext.Create(
          [ !"T1" |> TypeCheckScope.Empty.Resolve, (input1, Kind.Star) ]
          |> Map.ofList,
...
let ``LangNext-Unify unifies can look lookups up and fail on structure`` () =
  ...
  let actual =
    (TypeValue
      ...
      .run (
        UnificationContext.Create(
          [ !"T1" |> TypeCheckScope.Empty.Resolve,
            (TypeValue.CreateDecimal(), Kind.Star) ]
          |> Map.ofList,
          Ballerina.DSL.Next.Types.TypeChecker.Model.TypeExprEvalSymbols.Empty
        ),
        UnificationState.Empty
...
let ``LangNext-Unify unifies can look lookups up and fail on missing identifier``
  ...
  let actual =
    (TypeValue
      ...
      .run (
        UnificationContext.Create(
          [ !"T2" |> TypeCheckScope.Empty.Resolve, (input1, Kind.Star) ]
          |> Map.ofList,
          Ballerina.DSL.Next.Types.TypeChecker.Model.TypeExprEvalSymbols.Empty
        ),
        UnificationState.Empty
  ...
  let program
    ...
    state {
      do!
        EquivalenceClasses.Bind(
          b,
          PrimitiveType.Int32 |> TypeValue.CreatePrimitive |> Right,
          Location.Unknown
        )
      ...
      do!
        EquivalenceClasses.Bind(
          c,
          PrimitiveType.String |> TypeValue.CreatePrimitive |> Right,
          Location.Unknown
        )
      ...
      do!
        EquivalenceClasses.Bind(
          a,
          b |> TypeValue.Var |> TypeValue.CreateSet |> Right,
          Location.Unknown
        )
      ...
      do!
        EquivalenceClasses.Bind(
          a,
          c |> TypeValue.Var |> TypeValue.CreateSet |> Right,
          Location.Unknown
        )
    }
