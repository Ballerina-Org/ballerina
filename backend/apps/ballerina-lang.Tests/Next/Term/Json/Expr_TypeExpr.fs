...
let ``Assert Expr<TypeExpr> -> ToJson -> FromJson -> Expr<TypeExpr>``
  (expression: Expr<TypeExpr<ValueExt<unit>>, Identifier, ValueExt<unit>>)
  ...
  | Left json ->
    ...
    let parser: JsonValue -> Sum<Expr<TypeExpr<ValueExt<unit>>, Identifier, ValueExt<unit>>, Errors<Unit>> =
...
let ``Dsl:Terms:Expr.Lambda json round-trip`` () =
  ...
  let expected: Expr<TypeExpr<ValueExt<unit>>, Identifier, ValueExt<unit>> =
...
let ``Dsl:Terms:Expr.SumDes json round-trip`` () =
  ...
  let expected =
    Expr<TypeExpr<ValueExt<unit>>, Identifier, ValueExt<unit>>
      .SumDes(
...
let ``Dsl:Terms:Expr.Primitives json round-trip`` () =
  ...
  let expected: Expr<TypeExpr<ValueExt<unit>>, Identifier, ValueExt<unit>> =
    Expr.Primitive(PrimitiveValue.Int32 123)
