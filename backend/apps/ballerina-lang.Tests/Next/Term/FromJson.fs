...
let ``LangNext-FromJson-Expr - Lambda parses correctly`` () =
  ...
  | Sum.Left(Expr.Lambda(var, Expr.Primitive(PrimitiveValue.Int 42))) -> Assert.That(var.Name, Is.EqualTo("x"))
...
let ``LangNext-FromJson-Expr - TypeLambda parses correctly`` () =
  ...
  match parseExpr json with
  | Sum.Left(Expr.TypeLambda(tp, Expr.Primitive(PrimitiveValue.Int 42))) -> Assert.That(tp.Name, Is.EqualTo("T"))
  | other -> Assert.Fail $"Unexpected result: {other}"
...
let ``LangNext-FromJson-Expr - Apply parses correctly`` () =
  ...
  match parseExpr json with
  | Sum.Left(Expr.Apply(Expr.Lambda(var, Expr.Primitive(PrimitiveValue.Int 1)), Expr.Primitive(PrimitiveValue.Int 2))) ->
    Assert.That(var.Name, Is.EqualTo("x"))
  | other -> Assert.Fail $"Unexpected result: {other}"
...
let ``LangNext-FromJson-Expr - RecordCons parses correctly`` () =
  ...
  match parseExpr json with
  | Sum.Left(Expr.RecordCons fields) ->
    Assert.That(
      fields
      |> List.exists (fun (k, v) -> k = "foo" && v = Expr.Primitive(PrimitiveValue.Int 1)),
      Is.True
    )

    Assert.That(
      fields
      |> List.exists (fun (k, v) -> k = "bar" && v = Expr.Primitive(PrimitiveValue.String "baz")),
      Is.True
    )
  | other -> Assert.Fail $"Unexpected result: {other}"
...
let ``LangNext-FromJson-Expr - UnionCons parses correctly`` () =
  ...
  match parseExpr json with
  | Sum.Left(Expr.UnionCons("Foo", Expr.Primitive(PrimitiveValue.Int 42))) -> Assert.Pass()
  | other -> Assert.Fail $"Unexpected result: {other}"
...
let ``LangNext-FromJson-Expr-SumCons parses correctly`` () =
  let json = """{"kind":"sum","case":[3,{"kind":"int","int":"42"}]}"""

  match parseExpr json with
  | Sum.Left(Expr.SumCons(3, Expr.Primitive(PrimitiveValue.Int 42))) -> Assert.Pass()
  | other -> Assert.Fail $"Unexpected result: {other}"
...
let ``LangNext-FromJson-Expr-UnionDes parses correctly`` () =
  ...
  match parseExpr json with
  | Sum.Left(Expr.UnionDes handlers) ->
    Assert.That(handlers.ContainsKey "Foo", Is.True)
    Assert.That(handlers.ContainsKey "Bar", Is.True)
  | other -> Assert.Fail $"Unexpected result: {other}"
...
let ``LangNext-FromJson-Expr - TupleDes parses correctly`` () =
  ...
  let expected: Expr<TypeExpr> = Expr.TupleDes(Expr.Lookup "myTuple", 1)
