...
let ``SyntheticData delegates imported generators to build arguments`` () =
  ...
  let cartesianProduct (seqs: list<seq<Sum<_, _>>>) : seq<Sum<list<_>, _>> =
    let rec go (seqs: list<seq<Sum<_, _>>>) =
      match seqs with
      | [] -> seq { yield sum.Return [] }
      | first :: rest ->
        seq {
          for s in first do
            ...
            | Sum.Left x ->
              for restResult in go rest do
                ...
                | Sum.Right e -> yield Sum.Right e
                | Sum.Left tail -> yield sum.Return(x :: tail)
        }
  ...
  let generator =
    { ImportedGenerator.Id = typeId
      Generate =
        fun _ generate imported ->
          imported.Arguments
          |> List.map generate
          |> cartesianProduct
          |> Seq.map (sum.Map Value.Tuple) }
...
in (r, u, t, s, l)
  ...
  match Expr.TypeCheckString (context, _db_query_sym, _make_db_query_type) program with
  | Left(_expr, _typeValue, finalState) ->
    ...
    for _ in 1..3 do
      ...
      generate "R" |> Seq.head |> assertRecord rType
      generate "U" |> Seq.head |> assertUnion uType
      generate "T" |> Seq.head |> assertTuple tType
      generate "S" |> Seq.head |> assertSum sType
      generate "L" |> Seq.head |> assertList lType
  | Right err -> Assert.Fail $"Type checking failed: {err}"
...
[<Test>]
let ``SyntheticData prints all generated values for complex type`` () =
  let program =
    """
type R = { A:int32; B:bool; C:string; }
in type U = | A of int32 | B of string | C of bool
...
in type S = int32 + string + bool
in type L = List [int32]
in type Inner = { Rec:R; Tup:T; Choice:S; Union:U; List:L; }
in let r:R = { A=1; B=true; C="x"; }
in let u:U = A 1
in let t:T = (1, true, "x")
in let s:S = 1Of3 1
in let l:L = List::Nil [int32] ()
in let inner:Inner = { Rec=r; Tup=t; Choice=s; Union=u; List=l; }
in inner
    """
  ...
  | Left(_expr, typeValue, finalState) ->
    ...
    Console.WriteLine $"=== Total values: {count} ==="
    Console.WriteLine $"=== Total errors: {errors} ==="
    Assert.That(count, Is.EqualTo 27, "Should generate exactly 27 values")
  | Right err -> Assert.Fail $"Type checking failed: {err}"
