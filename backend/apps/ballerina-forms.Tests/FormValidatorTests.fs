...
let private makeTypeBindings (bindings: (string * ExprType) list) : Map<string, TypeBinding> =
  bindings
  |> List.map (fun (name, t) -> name, TypeBinding.Create(ExprTypeId.Create name, t))
  |> Map.ofList
...
let ``Record fails when renderer expects exact fields`` () =
          ...
          Renderer.RecordRenderer
            { Renderer = Some rendererName
              Fields =
                { Fields =
                    Map.ofList
                      [ "x", field "x" (ExprType.PrimitiveType PrimitiveType.IntType)
                        "y", field "y" (ExprType.PrimitiveType PrimitiveType.IntType) ]
  ...
  let ctx =
    newContext types (Map.ofList [ rendererName, Set.ofList [ "x" ] ]) (LanguageStreamType "Language")
  ...
  | Right(errs: Errors<unit>) ->
    Assert.That(Errors.ToString(errs, "\n"), Contains.Substring "form renderer expects exactly fields")

[<Test>]
let ``Record succeeds with matching fields and types`` () =
  let recordType =
    ExprType.RecordType(Map.ofList [ "x", ExprType.PrimitiveType PrimitiveType.IntType ])
...
let ``Record succeeds when field type is lookup and renderer matches underlying type`` () =
  ...
  let recordType =
    ExprType.RecordType(Map.ofList [ "id", ExprType.LookupType(ExprTypeId.Create "UserId") ])
...
let ``TranslationOverride succeeds with MapRenderer and proper key/value renderers`` () =
  ...
  let languageType =
    ExprType.RecordType(Map.ofList [ "key", ExprType.PrimitiveType PrimitiveType.StringType ])
  ...
  let types =
    makeTypeBindings [ languageTypeId.VarName, languageType; typeId.VarName, tOverride ]
  ...
  let keyRenderer: Renderer<unit, unit> =
    Renderer.StreamRenderer(
      StreamRendererApi.Stream(StreamApi.Id(StreamApi.Create("KeyStream", languageTypeId))),
      None,
  ...
  let ctx = newContext types Map.empty (LanguageStreamType languageTypeId.VarName)
  let result = FormBody.Validate ctx body

  match result with
  | Left et -> Assert.That(et, Is.EqualTo tOverride)
  | Right e -> Assert.Fail(sprintf "Expected success but got error: %s" (Errors.ToString(e, "\n")))
...
let ``Record with inline form renderer fails when inline form has wrong type`` () =
  ...
  match result with
  | Left _ -> Assert.Fail("Expected failure but got success")
  | Right(errs: Errors<unit>) -> Assert.That(Errors.ToString(errs, "\n"), Contains.Substring "cannot be unified")
