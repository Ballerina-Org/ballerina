...
let private extensionParser = (fun _ _ -> sum.Throw(Errors.Singleton $"Error"))
...
module ExprParserTests =
  ...
  let ``Should parse unit`` () =
    ...
    let result = Expr.Parse extensionParser json
...
//     let result = expr |> Expr.ToJson |> Sum.bind (Expr.Parse extensionParser)
