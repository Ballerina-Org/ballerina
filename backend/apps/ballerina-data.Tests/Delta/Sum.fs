...
let ``Delta.Sum: Updates correct case index in sum value`` () =
  let sumValue =
    Value<TypeValue<Unit>, Unit>
      .Sum({ Case = 1; Count = 1 }, PrimitiveValue.Int32 42 |> Value<TypeValue<Unit>, Unit>.Primitive)

  let delta =
    Delta.Sum(1, Delta.Replace(PrimitiveValue.Int32 100 |> Value<TypeValue<Unit>, Unit>.Primitive))
  ...
  | Sum.Left updater ->
    ...
    | Sum.Left(Value.Sum(updatedIndex, updatedValue)) ->
      ...
      Assert.That(updatedValue, Is.EqualTo(PrimitiveValue.Int32 100 |> Value<TypeValue<Unit>, Unit>.Primitive))
...
let ``Delta.Sum: Returns original value when index does not match`` () =
  let sumValue =
    Value<TypeValue<Unit>, Unit>
      .Sum({ Case = 2; Count = 1 }, PrimitiveValue.String "untouched" |> Value<TypeValue<Unit>, Unit>.Primitive)
...
let ``Delta.Sum: Fails when delta type does not match case type TODO:decide`` () =
  ...
  let delta =
    Delta.Sum(1, Delta.Replace(PrimitiveValue.String "wrong type" |> Value<TypeValue<Unit>, Unit>.Primitive))
