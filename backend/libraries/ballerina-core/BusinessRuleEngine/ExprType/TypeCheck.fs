      ...
      fun typeCheckExprExtension typeCheckValueExtension typeBindings vars e ->
        ...
        and typeCheckLambda
          ...
          sum {
            ...
            return! eval (vars |> Map.add x x_t) body
          }
        ...
        and typeCheckValue
          ...
          sum {
            ...
            | Value.Lambda(x, body) -> return! typeCheckLambda vars x body
            | Value.Extension e ->
              return!
        ...
        and typeCheckApplication
          ...
          sum {
            ...
            match f with
            | Expr.Value(Value.Lambda(x, body)) -> return! eval (vars |> Map.add x arg) body
            | _ ->
              let! f = !f
