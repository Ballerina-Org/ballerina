...
module Lexer =
  ...
  let keyword =
    tokenizer {
      let! res =
        tokenizer.Any
          [ word (View.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword View)
            word (From.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword From)
            ...
            word (Keyword.Date.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Date)
            word (Keyword.DateOnly.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword DateOnly)
            word (Keyword.Guid.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Guid)
            word (Keyword.Bool.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Bool)
            ...
            word (Keyword.Unit.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Unit)
            word (Keyword.Table.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Table)
            word (Tab.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Tab)
            word (Column.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Column)
            word (Group.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Group)
            word (With.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword With)
            word (In.ToString()) |> tokenizer.Map(LocalizedToken.FromKeyword In)
            word (Keyword.Enum.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Enum)
            word (Keyword.Single.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Single)
            word (Multi.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Multi)
            word (Renderer.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Renderer)
            word (Disable.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Disable)
            word (Keyword.Tuple.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Tuple)
            word (Stream.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Stream)
            word (Keyword.List.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword List)
            word (Union.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Union)
            word (Keyword.Option.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Option)
            word (Record.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Record)
            word (Sum.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Sum)
            word (Tooltip.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Tooltip)
            word (Create.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Create)
            word (Edit.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Edit)
            word (Passthrough.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Passthrough)
            word (Launcher.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Launcher)
            word (Details.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Details)
            word (Keyword.Map.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Map)
            word (ReadOnly.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword ReadOnly)
            ...
            word (One.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword One)
            word (Many.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Many)
            word (Preview.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Preview)
            word (Linked.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Linked)
            word (Unlinked.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Unlinked)
            word (Element.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Element)
            word (Add.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Add)
            word (Remove.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Remove)
            word (Clear.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Clear)
            word (Move.ToString())
            |> tokenizer.Map(LocalizedToken.FromKeyword Move)
      ...
      do!
        tokenizer.Lookahead(
          tokenizer.Exactly(
            (fun c -> c |> Char.IsLetterOrDigit || c = '_') >> not
          )
          |> tokenizer.Ignore
        )
  ...
  let operator =
    tokenizer.Any
      [ word ":" |> tokenizer.Map(LocalizedToken.FromOperator Colon)
        ...
        word "{"
        |> tokenizer.Map(LocalizedToken.FromOperator(CurlyBracket Open))
        word "}"
        |> tokenizer.Map(LocalizedToken.FromOperator(CurlyBracket Close))
        word "["
        |> tokenizer.Map(LocalizedToken.FromOperator(SquareBracket Open))
        word "]"
        |> tokenizer.Map(LocalizedToken.FromOperator(SquareBracket Close))
        word "("
        |> tokenizer.Map(LocalizedToken.FromOperator(RoundBracket Open))
        word ")"
        |> tokenizer.Map(LocalizedToken.FromOperator(RoundBracket Close))
        ...
        word (Arrow.ToString())
        |> tokenizer.Map(LocalizedToken.FromOperator Arrow)
        word (Choice1.ToString())
        |> tokenizer.Map(LocalizedToken.FromOperator Choice1)
        word (Choice2.ToString())
        |> tokenizer.Map(LocalizedToken.FromOperator Choice2)
  ...
  let identifier =
    tokenizer {
      ...
      let! rest =
        [ letter; digit; underscore ] |> tokenizer.Any |> tokenizer.Many
