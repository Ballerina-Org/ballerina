...
module TypeValue =
  ...
  type TypeValue<'valueExt> with
    static member FromJson
      (json: JsonValue)
      : Sum<TypeValue<'valueExt>, Errors<_>> =
      ...
      let inline parse
        ...
        json
        =
        WithSourceMapping.FromJson
          (fromJson TypeValue.FromJson)
          typeExprFromJson
          json
        |> sum.Map ctor
      ...
      sum.Any(
        ...
        [ TypeValue.FromJsonVar(json)
          ...
          (WithSourceMapping.FromJson
            (TypeValue.FromJsonLambda typeExprFromJson)
            typeExprFromJson
            json
    ...
    static member ToJson(v: TypeValue<'valueExt>) : JsonValue =
      ...
      let inline serialize makeJson mapping =
        WithSourceMapping<_, _>.ToJson
          (makeJson TypeValue.ToJson)
          typeExprToJson
          mapping

      match v with
      | TypeValue.Primitive primitive ->
        TypeValue.ToJsonPrimitive primitive.value
      ...
      | TypeValue.Lambda mapping ->
        WithSourceMapping<TypeParameter * TypeExpr<'valueExt>, 'valueExt>.ToJson
          ...
          typeExprToJson
          mapping
      | TypeValue.Application mapping ->
        serialize TypeValue.ToJsonApplication mapping
      ...
      | TypeValue.Entities _ ->
        failwith "Schema Entities ToJson not implemented"
      | TypeValue.Relations _ ->
        failwith "Schema Relations ToJson not implemented"
      | TypeValue.Relation _ ->
        failwith "Schema Relation ToJson not implemented"
      | TypeValue.RelationLookupOption _ ->
        failwith "Schema LookupMaybe ToJson not implemented"
      | TypeValue.RelationLookupOne _ ->
        failwith "Schema LookupOne ToJson not implemented"
      | TypeValue.RelationLookupMany _ ->
        failwith "Schema LookupMany ToJson not implemented"
      | TypeValue.ForeignKeyRelation _ ->
        failwith "Schema ForeignKeyRelation ToJson not implemented"
      | TypeValue.QueryTypeFunction ->
        failwith "QueryTypeFunction ToJson not implemented"
