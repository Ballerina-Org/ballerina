    ...
    static member FromJson: JsonValue -> Sum<TypeCheckScope, Errors> =
      fun scope ->
        sum {
          ...
          let! assembly = fields |> Map.tryFindWithError "assembly" "TypeCheckScope" "assembly"
    ...
    static member FromJson
      ...
      fun mapping ->
        sum {
          ...
          let! ty = mapping |> Map.tryFindWithError "type" "TypeExprSourceMapping" "type"
          let! ty = JsonValue.AsString ty
          let! value = mapping |> Map.tryFindWithError "value" "TypeExprSourceMapping" "value"
          ...
          | "originTypeExprLet" ->
            ...
            let! bindingName =
              fields
              |> Map.tryFindWithError "bindingName" "originTypeExprLet" "bindingName"
              |> Sum.bind JsonValue.AsString

            let! typeExpr =
              fields
              |> Map.tryFindWithError "typeExpr" "originTypeExprLet" "ftypeExprelds"
    ...
    static member FromJson
      ...
      Sum.assertDiscriminatorAndContinueWithValue discriminator (fun withSourceMapping ->
        sum {
          ...
          let! scope =
            withSourceMapping
            |> Map.tryFindWithError "typeCheckScopeSource" "WithSourceMapping" "typeCheckScopeSource"

          let! scope = TypeCheckScope.FromJson scope

          let! typeExpr =
            withSourceMapping
            |> Map.tryFindWithError "typeExprSource" "WithSourceMapping" "typeExprSource"

          let! typeExpr = TypeExprSourceMapping.FromJson typeExprFromJson typeExpr
          let! value = withSourceMapping |> Map.tryFindWithError "value" "WithSourceMapping" "value"
