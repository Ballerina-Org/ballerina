...
module Patterns =
  ...
  type ListOperations<'ext> with
    static member AsMap(op: ListOperations<'ext>) : Sum<Option<Value<TypeValue<'ext>, 'ext>>, Errors<Unit>> =
      ...
      | _ ->
        (fun () -> $"Error: Expected List_Map, found {op}")
        |> Errors.Singleton()
        |> sum.Throw
    ...
    static member AsAppend(op: ListOperations<'ext>) : Sum<Option<Value<TypeValue<'ext>, 'ext>>, Errors<Unit>> =
      ...
      | _ ->
        (fun () -> $"Error: Expected List_Append, found {op}")
        |> Errors.Singleton()
        |> sum.Throw
    ...
    static member AsFilter(op: ListOperations<'ext>) : Sum<Option<Value<TypeValue<'ext>, 'ext>>, Errors<Unit>> =
      ...
      | _ ->
        (fun () -> $"Error: Expected List_Filter, found {op}")
        |> Errors.Singleton()
        |> sum.Throw
    ...
    static member AsFold
      ...
      | _ ->
        (fun () -> $"Error: Expected List_Fold, found {op}")
        |> Errors.Singleton()
        |> sum.Throw

    static member AsLength(op: ListOperations<'ext>) : Sum<Unit, Errors<Unit>> =
      match op with
      | List_Length -> () |> sum.Return
      | _ ->
        (fun () -> $"Error: Expected List_Length, found {op}")
        |> Errors.Singleton()
        |> sum.Throw

    static member AsCons(op: ListOperations<'ext>) : Sum<Unit, Errors<Unit>> =
      match op with
      | List_Cons -> () |> sum.Return
      | _ ->
        (fun () -> $"Error: Expected List_Cons, found {op}")
        |> Errors.Singleton()
        |> sum.Throw

    static member AsNil(op: ListOperations<'ext>) : Sum<Unit, Errors<Unit>> =
      match op with
      | List_Nil -> () |> sum.Return
      | _ ->
        (fun () -> $"Error: Expected List_Nil, found {op}")
        |> Errors.Singleton()
        |> sum.Throw
