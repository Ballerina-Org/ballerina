...
type internal Utils private () =
  static member CastByteToSingle
    (fromMemory: Memory<byte>)
    : ReadOnlyMemory<single> =
    ReadOnlyMemory<single>(
      MemoryMarshal.Cast<byte, single>(fromMemory.Span).ToArray()
    )

  static member CastByteToSByte
    (fromMemory: Memory<byte>)
...
type EmbeddingF32 =
  ...
  static member GetBufferByteLength(dimensions: int) : int =
    dimensions * sizeof<single>
