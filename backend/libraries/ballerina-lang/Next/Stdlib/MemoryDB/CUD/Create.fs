...
module Create =
  ...
  let onCreatingHook<'ext when 'ext: comparison>
    (calculateProps:
  ...
  let onCreatedHook<'ext when 'ext: comparison>
    (_calculateProps:
      Value<TypeValue<'ext>, 'ext>
        -> SchemaEntity<'ext>
        -> Reader<Value<TypeValue<'ext>, 'ext>, ExprEvalContext<'ext>, Errors<Location>>)
            ...
            reader {
              ...
              | Some(_schema, _db, _entity, _schema_as_value) -> // the closure has the first operand - second step in the application
                ...
                | [ _entityId; v ] ->
                  ...
                  let! valueWithProps =
                    onCreatingHook calculateProps _entity _schema_as_value.Value.Value loc0 _entityId v valueWithProps

                  do addEntityValue _db _entity _entityId valueWithProps

                  do! onCreatedHook calculateProps _entity _schema_as_value.Value.Value loc0 _entityId v valueWithProps
