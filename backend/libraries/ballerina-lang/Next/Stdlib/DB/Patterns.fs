...
module Patterns =
  ...
  type DBValues<'runtimeContext, 'db, 'ext when 'ext: comparison> with
    static member AsRun(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<Unit, Errors<Unit>> =
      match op with
      | DBValues.Run -> () |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected Run operation") |> sum.Throw

    static member AsTypeAppliedRun(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<Schema<'ext> * 'db, Errors<Unit>> =
      match op with
      | DBValues.TypeAppliedRun(s, db) -> (s, db) |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected TypeAppliedRun operation") |> sum.Throw

    static member AsEvalProperty(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<DBEvalProperty<'ext>, Errors<Unit>> =
      match op with
      | DBValues.EvalProperty v -> v |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected EvalProperty operation") |> sum.Throw

    static member AsStripProperty(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<DBEvalProperty<'ext>, Errors<Unit>> =
      match op with
      | DBValues.StripProperty v -> v |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected StripProperty operation") |> sum.Throw

    static member AsCreate(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<Option<EntityRef<'db, 'ext>>, Errors<Unit>> =
      match op with
      | DBValues.Create v -> v.EntityRef |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected Create operation") |> sum.Throw

    static member AsUpdate(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<Option<EntityRef<'db, 'ext>>, Errors<Unit>> =
      match op with
      | DBValues.Update v -> v.EntityRef |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected Update operation") |> sum.Throw

    static member AsUpsert(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<Option<EntityRef<'db, 'ext>>, Errors<Unit>> =
      match op with
    ...
    static member AsDelete(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<Option<EntityRef<'db, 'ext>>, Errors<Unit>> =
      match op with
    ...
    static member AsEntityRef(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<EntityRef<'db, 'ext>, Errors<Unit>> =
      match op with
      | DBValues.EntityRef e -> e |> sum.Return
      | _ -> Errors.Singleton () (fun () -> "Expected EntityRef operation") |> sum.Throw

    static member AsRelationRef(op: DBValues<'runtimeContext, 'db, 'ext>) : Sum<RelationRef<'db, 'ext>, Errors<Unit>> =
      match op with
      | DBValues.RelationRef(s, db, r, f, t, sv) -> (s, db, r, f, t, sv) |> sum.Return
    ...
    static member AsIsLinked
      ...
      | _ -> Errors.Singleton () (fun () -> "Expected IsLinked operation") |> sum.Throw
    ...
    static member AsLookupOne
      (op: DBValues<'runtimeContext, 'db, 'ext>)
      : Sum<{| RelationRef: Option<RelationRef<'db, 'ext> * RelationLookupDirection> |}, Errors<Unit>> =
      match op with
