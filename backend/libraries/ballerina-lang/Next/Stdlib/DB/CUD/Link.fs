...
module Link =
  ...
  let onLinkingHook
    ...
    reader {
      ...
      match relation.Hooks.OnLinking with
      | None -> return ()
  ...
  let onLinkedHook
    ...
    reader {
      ...
      match relation.Hooks.OnLinked with
      | None -> return ()
      | Some(hookExpr: TypeCheckedExpr<'ext>) ->
        let! run_hook_result =
          TypeCheckedExpr.UnsafeApplyForUntypedEval(
  ...
  let DBLinkExtension<'runtimeContext, 'db, 'ext when 'ext: comparison>
      ...
      { PublicIdentifiers =
        ...
        OperationsLens =
          ...
          |> PartialLens.BindGet (function
            | DBValues.Link v -> Some(DBValues.Link v)
            | _ -> None)
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
  ...
  let MemoryDBLinkManyExtension<'runtimeContext, 'db, 'ext when 'ext: comparison>
    (db_ops: DBTypeClass<'runtimeContext, 'db, 'ext>)
