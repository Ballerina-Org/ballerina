...
module TypeCheckedExprLegacyConstructors =
  ...
  type TypeCheckedExpr<'valueExt> with
    ...
    static member UnsafeApplyForUntypedEval(f: TypeCheckedExpr<'valueExt>, a: TypeCheckedExpr<'valueExt>) =
      TypeCheckedExpr<'valueExt>.Apply(f, a, TypeValue.CreateUnit(), Kind.Star)

    static member UnsafeLambdaForUntypedEval
      (v: Var, t: TypeValue<'valueExt>, e: TypeCheckedExpr<'valueExt>, r: TypeValue<'valueExt>)
      =
      TypeCheckedExpr<'valueExt>.Lambda(v, t, e, r, TypeValue.CreateArrow(t, r), Kind.Star)

    static member UnsafeQueryForUntypedEval(q: TypeCheckedExprQuery<'valueExt>) =
    ...
    static member UnsafeSumConsForUntypedEval(selector: SumConsSelector) =
      TypeCheckedExpr<'valueExt>.SumCons(selector, TypeValue.CreateUnit(), Kind.Star)

    static member UnsafeTupleConsForUntypedEval(items: List<TypeCheckedExpr<'valueExt>>) =
      TypeCheckedExpr<'valueExt>.TupleCons(items, TypeValue.CreateUnit(), Kind.Star)

    static member UnsafeRecordConsForUntypedEval(fields: List<ResolvedIdentifier * TypeCheckedExpr<'valueExt>>) =
      TypeCheckedExpr<'valueExt>.RecordCons(fields, TypeValue.CreateUnit(), Kind.Star)
