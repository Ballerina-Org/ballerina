...
module Extension =
  ...
  let Float32Extension<'runtimeContext, 'ext>
    ...
    let float32PlusId =
      Identifier.FullyQualified([ "float32" ], "+")
    ...
    let toStringOperation
      : ResolvedIdentifier *
        OperationExtension<'runtimeContext, 'ext, Float32Operations<'ext>> =
      float32ToStringId,
      { PublicIdentifiers =
          Some
          <| (TypeValue.CreateArrow(float32TypeValue, stringTypeValue),
              Kind.Star,
              Float32Operations.String)
        OperationsLens =
          operationLens
            ...
            reader {
              ...
              return
                ...
                | true, result ->
                  Value.Sum(
                    { Case = 2; Count = 2 },
                    Value.Primitive(PrimitiveValue.Float32 result)
                  )
                | false, _ ->
                  Value.Sum(
                    { Case = 1; Count = 2 },
    ...
    let plusOperation
      ...
      { PublicIdentifiers =
          ...
          <| (TypeValue.CreateArrow(
                float32TypeValue,
                TypeValue.CreateArrow(float32TypeValue, float32TypeValue)
              ),
              Kind.Star,
          ...
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.Plus({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32PlusId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
                ...
                return
                  Value<TypeValue<'ext>, 'ext>
                    .Primitive(PrimitiveValue.Float32(vClosure + v))
            } }
        ...
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.Minus({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32MinusId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
        ...
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.Divide({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32DivideId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
    ...
    let powerOperation
      ...
      { PublicIdentifiers =
          ...
          <| (TypeValue.CreateArrow(
                float32TypeValue,
                TypeValue.CreateArrow(int32TypeValue, float32TypeValue)
              ),
              Kind.Star,
          ...
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                ...
                return
                  (Float32Operations.Power({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32PowerId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
                ...
                return
                  Value<TypeValue<'ext>, 'ext>
                    .Primitive(PrimitiveValue.Float32(pown vClosure v))
            } }
        ...
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.Mod({| v1 = Some v |}) |> operationLens.Set,
                   Some float32ModId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
    ...
    let equalOperation
      ...
      { PublicIdentifiers =
          ...
          <| (TypeValue.CreateArrow(
                float32TypeValue,
                TypeValue.CreateArrow(float32TypeValue, boolTypeValue)
              ),
              Kind.Star,
        ...
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.Equal({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32EqualId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
        ...
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.NotEqual({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32NotEqualId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
    ...
    let greaterThanOperation
      ...
      { PublicIdentifiers =
        ...
        OperationsLens =
          operationLens
          |> PartialLens.BindGet (function
            | Float32Operations.GreaterThan v ->
              Some(Float32Operations.GreaterThan v)
            | _ -> None)
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.GreaterThan({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32GreaterThanId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
    ...
    let greaterThanOrEqualOperation
      ...
      { PublicIdentifiers =
        ...
        OperationsLens =
          operationLens
          |> PartialLens.BindGet (function
            | Float32Operations.GreaterThanOrEqual v ->
              Some(Float32Operations.GreaterThanOrEqual v)
            | _ -> None)
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
        ...
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.LessThan({| v1 = Some v |})
                   |> operationLens.Set,
                   Some float32LessThanId)
                  |> Ext
              | Some vClosure -> // the closure has the first operand - second step in the application
    ...
    let lessThanOrEqualOperation
      ...
      { PublicIdentifiers =
        ...
        OperationsLens =
          operationLens
          |> PartialLens.BindGet (function
            | Float32Operations.LessThanOrEqual v ->
              Some(Float32Operations.LessThanOrEqual v)
            | _ -> None)
        Apply =
          fun loc0 _rest (op, v) ->
            reader {
              ...
              | None -> // the closure is empty - first step in the application
                return
                  (Float32Operations.LessThanOrEqual({| v1 = Some v |})
                   |> operationLens.Set,
