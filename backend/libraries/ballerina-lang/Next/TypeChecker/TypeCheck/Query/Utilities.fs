...
module QueryUtilities =
  ...
  let two_equal_primitives loc0 expected_primitive_type (t, (e: TypeCheckedExprQueryExpr<_>)) =
    match t, e.Expr with
    ...
    | [ TypeQueryRow.Json(TypeValue.Primitive { value = p1 }); TypeQueryRow.PrimitiveType(p2, is_p2_nullable) ],
      ...
      ->
      Some(
        [ TypeCheckedExprQueryExprRec.QueryCastTo(v1, TypeQueryRow.PrimitiveType(expected_primitive_type, false))
    ...
    | [ TypeQueryRow.PrimitiveType(p1, is_p1_nullable); TypeQueryRow.Json(TypeValue.Primitive { value = p2 }) ],
      ...
      Some(
        [ v1
          TypeCheckedExprQueryExprRec.QueryCastTo(v2, TypeQueryRow.PrimitiveType(expected_primitive_type, false))
          |> TypeCheckedExprQueryExpr.Create loc0 ]
        |> TypeCheckedExprQueryExprRec.QueryTupleCons
        |> TypeCheckedExprQueryExpr.Create loc0,
        is_p1_nullable
