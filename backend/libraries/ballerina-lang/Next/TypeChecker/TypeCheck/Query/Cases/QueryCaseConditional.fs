  ...
  let typeCheckQueryConditional<'valueExt when 'valueExt: comparison>
    ...
    state {
      ...
      let! cond_e =
        ...
        | _ ->
          (fun () ->
            $"Type checking error: condition of if expression in query must be bool, but got {cond_t}")
      ...
      let! then_e_final, else_e_final, result_t =
        ...
        | TypeQueryRow.Json(TypeValue.Primitive { value = pt }),
          TypeQueryRow.PrimitiveType(pt2, n) when pt = pt2 ->
          state {
        ...
        | TypeQueryRow.PrimitiveType(pt, n),
          TypeQueryRow.Json(TypeValue.Primitive { value = pt2 }) when pt = pt2 ->
          state {
            let cast_t = TypeQueryRow.PrimitiveType(pt, false)
      ...
      return
        TypeCheckedExprQueryExprRec.QueryConditional(
          cond_e,
          then_e_final,
          else_e_final
        )
