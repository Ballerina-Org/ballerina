...
module ApplyValue =
  ...
  type Expr<'T, 'Id, 'valueExt when 'Id: comparison> with
    static member internal TypeCheckApplyValue
      (typeCheckExpr: ExprTypeChecker<'valueExt>, loc0: Location)
      : TypeChecker<ExprApplyValue<TypeExpr, Identifier, 'valueExt>, 'valueExt> =
      fun context_t ({ F = f_expr; Arg = a; ArgT = aT }) ->
        ...
        state {
          let f = f_expr
          let a, t_a, a_k = a, aT, Kind.Star
          do! a_k |> Kind.AsStar |> ofSum |> state.Ignore
          let! (ctx: TypeCheckContext) = state.GetContext()
