...
module ValueSerializer =
  ...
  let rec recordToDTO (record: Map<ResolvedIdentifier, Value<'T, 'valueExt>>) =
    record
    |> Map.toList
    |> List.sortWith (fun (id1, _) (id2, _) ->
      ResolvedIdentifier.Compare id1 id2)
  ...
  and unionCaseToDTO
    ((identifier, value): ResolvedIdentifier * Value<'T, 'valueExt>)
    =
    reader {
      let identifierDTO = identifier.ToDTO
      let! valueDTO = valueToDTO value
  ...
  and valueToDTO
    ...
    reader {
      ...
      | UnionCase(identifier, value) ->
        return! unionCaseToDTO (identifier, value)
      | RecordDes identifier ->
        return new ValueDTO<'valueExtDTO>(identifier.ToDTO)
      ...
      | Primitive primitive ->
        return new ValueDTO<'valueExtDTO>(primitiveToDTO primitive)
        ...
        return!
          reader.Throw(
            Errors.Singleton () (fun _ ->
              $"The value {value} cannot be converted to DTO.")
          )
    }
  ...
  type Value<'T, 'valueExt> with
    static member JsonSerializeV2(value: Value<'T, 'valueExt>) =
      reader {
        ...
        return
          JsonSerializer.Serialize(valueDTO, jsonSerializationConfiguration)
      }
