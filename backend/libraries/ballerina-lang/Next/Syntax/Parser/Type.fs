...
module Type =
  ...
  type ComplexType =
    ...
    | BinaryExpressionChain of NonEmptyList<BinaryOperator * TypeExpr>
            ...
            (fun acc e ->
              sum {
                ...
                | BinaryExpressionChain fields ->
                  let fields: List<BinaryOperatorsElement<TypeExpr, BinaryOperator>> =
                    fields
                    |> NonEmptyList.ToList
                  ...
                  let precedence: List<OperatorsPrecedence<BinaryOperator>> =
                    [ { Operators = [ BinaryOperator.Times ] |> Set.ofList
                        Associativity = AssociateLeft }
                      { Operators = [ BinaryOperator.Plus ] |> Set.ofList
                        Associativity = AssociateLeft } ]
