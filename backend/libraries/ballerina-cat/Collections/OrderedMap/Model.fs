...
module OrderedMap =
  ...
  type OrderedMap<'K, 'V when 'K: comparison> =
    ...
    // same as Map
    static member empty: OrderedMap<'K, 'V> =
      { reverseOrder = []; data = Map.empty }
    ...
    static member containsKey (om: OrderedMap<'K, 'V>) (k: 'K) =
      om.data.ContainsKey k

    static member tryFind (k: 'K) (om: OrderedMap<'K, 'V>) = om.data.TryFind k

    static member keys(om: OrderedMap<'K, 'V>) : List<'K> =
      om.reverseOrder |> List.rev
    ...
    // new methods
    static member addIfNotExists
      (om: OrderedMap<'K, 'V>)
      (k: 'K)
      (v: 'V)
      : Option<OrderedMap<'K, 'V>> =
      if om.data.ContainsKey k then
        None
      else
        Some(OrderedMap.Add om k v)

    static member mergeSecondAfterFirst
      (om1: OrderedMap<'K, 'V>)
      (om2: OrderedMap<'K, 'V>)
      : OrderedMap<'K, 'V> =
      ...
      let mergedData =
        om1.data
        |> Map.fold (fun (acc: Map<'K, 'V>) k v -> acc.Add(k, v)) om2.data
