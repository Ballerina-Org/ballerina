...
module WithError =
  ...
  type Reader<'a, 'c, 'e> =
    ...
    static member ofSum(r: Sum<'a, 'e>) : Reader<'a, 'c, 'e> =
      Reader(fun _ -> r)
    ...
    static member map<'b>
      (f: 'a -> 'b)
      : (Reader<'a, 'c, 'e> -> Reader<'b, 'c, 'e>) =
      fun (Reader r) -> Reader(r >> Sum.map f)

    static member mapError<'e1>
      (f: 'e -> 'e1)
      : (Reader<'a, 'c, 'e> -> Reader<'a, 'c, 'e1>) =
      fun (Reader r) ->
        Reader(fun ctx ->
          match r ctx with
          | Left res -> Left res
    ...
    static member mapContext<'c1>
      (f: 'c1 -> 'c)
      : (Reader<'a, 'c, 'e> -> Reader<'a, 'c1, 'e>) =
    ...
    static member cons: 'a -> Reader<'a, 'c, 'e> =
      fun v -> Reader(fun _c -> sum { return v })

    static member throw: 'e -> Reader<'a, 'c, 'e> =
      fun e -> Reader(fun _c -> sum.Throw(e))

  type ReaderBuilder() =
    member _.Return<'c, 'a, 'e>(v: 'a) : Reader<'a, 'c, 'e> = Reader.cons v
    member _.ReturnFrom<'c, 'a, 'e>(r: Reader<'a, 'c, 'e>) = r

    member _.Bind<'c, 'a, 'b, 'e>
      (r: Reader<'a, 'c, 'e>, f: 'a -> Reader<'b, 'c, 'e>)
      =
      r |> Reader.map f |> Reader.join

    member reader.Combine<'c, 'a, 'b, 'e>
      (r1: Reader<'a, 'c, 'e>, r2: Reader<'b, 'c, 'e>)
      =
      reader {
        let! _ = r1
        return! r2
    ...
    member reader.Throw<'c, 'a, 'e>(error: 'e) : Reader<'a, 'c, 'e> =
      Reader(fun _ -> sum.Throw(error))

    member inline _.All2<'c, 'a, 'e
    ...
    member inline _.AllNonEmpty<'c, 'a, 'e
      when 'e: (static member Concat: 'e * 'e -> 'e)>
    ...
    member inline reader.AllMap<'c, 'a, 'e, 'k
      ...
      Reader(fun (c: 'c) ->
        sum {
          let! (results: Map<'k, 'a>) =
            readers |> Map.map (fun _k (Reader p) -> p c) |> sum.AllMap
    ...
    member _.MapContext<'c1, 'c, 'a, 'e>
      (f: 'c1 -> 'c)
      : Reader<'a, 'c, 'e> -> Reader<'a, 'c1, 'e> =
      Reader.mapContext f

    member _.Map<'c, 'a, 'b, 'e>
      (f: 'a -> 'b)
      : Reader<'a, 'c, 'e> -> Reader<'b, 'c, 'e> =
      Reader.map f

    member _.MapError<'c, 'a, 'e, 'e1>
      (f: 'e -> 'e1)
      : Reader<'a, 'c, 'e> -> Reader<'a, 'c, 'e1> =
      Reader.mapError f

    member _.Ignore<'c, 'a, 'e>(r: Reader<'a, 'c, 'e>) : Reader<Unit, 'c, 'e> =
      Reader.map (fun _ -> ()) r

    member _.GetContext() : Reader<'c, 'c, 'e> =
      Reader(fun c -> sum { return c })
