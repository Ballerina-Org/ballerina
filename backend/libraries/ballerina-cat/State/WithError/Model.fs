...
module WithError =
    ...
    static member mapState<'sOuter>
      ...
      State(
        FreeNode.fromStep (fun (c, s0: 'sOuter) ->
          ...
          | Left(res, u_s) ->
            Left(res, u_s |> Option.map (fun s -> Δ (s, c) s0))
  ...
  type StateBuilder() =
    ...
    member _.Zero<'c, 's, 'e>() : State<unit, 'c, 's, 'e> =
      State(FreeNode.Return())

    member _.Return<'a, 'c, 's, 'e>(result: 'a) : State<'a, 'c, 's, 'e> =
      State(FreeNode.Return result)

    member _.Yield<'a, 'c, 's, 'e>(result: 'a) : State<'a, 'c, 's, 'e> =
      State(FreeNode.Return result)
    ...
    member _.Catch
      ((State p): State<'a, 'c, 's, 'e>)
      : State<Sum<'a, 'e>, 'c, 's, 'e> =
      State(FreeNode.catch p)

    member _.Throw(e: 'e) = State(FreeNode.throw e)

    member state.Delay<'a, 'c, 's, 'e>
      (p: unit -> State<'a, 'c, 's, 'e>)
      : State<'a, 'c, 's, 'e> =
      p ()
      ...
      state {
        ...
        | NonEmptyList(p, ps) ->
          ...
          | Right e1 ->
            ...
            | p' :: ps' ->
              return! state.AnyAcc(e, e1 |> Some, NonEmptyList.OfList(p', ps'))
      }
    ...
    member inline state.AllMap<'k, 'a, 'c, 's, 'b
    ...
    member state.OfReader<'a, 'c, 's, 'e>
      (Reader r: Reader<'a, 'c, 'e>)
      : State<'a, 'c, 's, 'e> =
      State(
    ...
    member state.OfStateReader<'a, 'c, 's, 'e>
      (Reader r: Reader<'a, 's, 'e>)
      : State<'a, 'c, 's, 'e> =
      State(
    ...
    member inline state.All4 p1 p2 p3 p4 =
      state.All2 p1 (state.All2 p2 (state.All2 p3 p4))
    ...
    member inline state.Either6 p1 p2 p3 p4 p5 p6 =
      state.Either
        ...
        (state.Either
          p2
          (state.Either p3 (state.Either p4 (state.Either p5 p6))))
    ...
    member inline state.Either7 p1 p2 p3 p4 p5 p6 p7 =
      state.Either
        ...
        (state.Either
          ...
          (state.Either
            p3
            (state.Either p4 (state.Either p5 (state.Either p6 p7)))))
      ...
      state.Either
        ...
        (state.Either
          ...
          (state.Either
            ...
            (state.Either
              p4
              (state.Either p5 (state.Either p6 (state.Either p7 p8))))))
