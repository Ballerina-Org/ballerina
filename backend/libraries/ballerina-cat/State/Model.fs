...
module Simple =
  ...
  type StateBuilder() =
    member _.Zero<'c, 's>() = State.fromValue<'c, 's> ()
    member _.Return<'a, 'c, 's>(result: 'a) = State.fromValue<'c, 's> result
    member _.Yield(result: 'a) = State.fromValue<'c, 's> result
