...
def _case_to_json(discriminator: int, payload: Json) -> Json:
    return [str(discriminator), payload]


def _case_from_json(case_payload: Json) -> Sum[ParsingError, tuple[str, Json]]:
    match case_payload:
...
def _handle_case_tuple(
    case_tuple: tuple[str, Json], left_from_json: FromJson[_SumL], right_from_json: FromJson[_SumR]
