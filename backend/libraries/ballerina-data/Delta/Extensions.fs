...
module Extensions =
  ...
  type Value = Value<TypeValue<ValueExt>, ValueExt>
  ...
  type DeltaExt with
    static member ToUpdater: (DeltaExt -> Value -> Sum<Value, Errors<unit>>) =
      fun (delta: DeltaExt) ->
        fun (value: Value) ->
          sum {
            match value with
            | Value.Ext(ValueExt(Choice1Of6(ListValues(List l))), _) ->
              match delta with
              | DeltaExt.DeltaExt(Choice1Of3(UpdateElement(i, v))) ->
                let next = List.updateAt i v l
                return (ValueExt(Choice1Of6(ListValues(List next))), None) |> Value.Ext
            ...
            | other ->
              return!
                sum.Throw(
                  Errors.Singleton () (fun () -> $"Expected value to be a list for List Delta ext, got {other}")
                )
          }
